package com.promote.mq.consumer;
...
import com.promote.mq.constants.RocketMQErrorEnum;
import com.promote.mq.consumer.processor.MQConsumeMsgListenerProcessor;
import com.promote.mq.exception.RocketMQException;
...
public class MQConsumerConfiguration {
    ...
    @Bean
    public DefaultMQPullConsumer getRocketMQPullConsumer() throws MQClientException, RemotingException, InterruptedException, MQBrokerException {
        ...
        consumer.start();
        Set<MessageQueue> messageQueues = consumer.fetchSubscribeMessageQueues(topics);
        consumer.fetchMessageQueuesInBalance("");
        for (MessageQueue mq:messageQueues){
            while (true){
                PullResult pullResult = consumer.pullBlockIfNotFound(mq,null,1,32);
    ...
    @Bean
    public DefaultMQPushConsumer getRocketMQPushConsumer() throws RocketMQException {
        if (StringUtils.isEmpty(groupName)){
            throw new RocketMQException(RocketMQErrorEnum.PARAMM_NULL,"groupName is null !!!",false);
        }
        if (StringUtils.isEmpty(namesrvAddr)){
